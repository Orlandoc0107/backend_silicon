...
const getAllProducts = async (req, res) => {
    try {
        const products = await MyPool.query('SELECT * FROM productos');
...
const getProductById = async (req, res) => {
    const { id_producto } = req.params;;
    try {
        const product = await MyPool.query('SELECT * FROM productos WHERE Id_producto = $1', [id_producto]);
...
const deleteProduct = async (req, res) => {
    const { id_producto } = req.params;
    try {
        const deletedProduct = await MyPool.query('DELETE FROM products WHERE id = $1 RETURNING *',[id_producto]);
